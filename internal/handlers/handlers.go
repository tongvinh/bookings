...
import (
	...
	"github.com/tongvinh/bookings/internal/config"
	"github.com/tongvinh/bookings/internal/driver"
	"github.com/tongvinh/bookings/internal/forms"
	"github.com/tongvinh/bookings/internal/helpers"
	"github.com/tongvinh/bookings/internal/models"
	"github.com/tongvinh/bookings/internal/render"
	"github.com/tongvinh/bookings/internal/repository"
	"github.com/tongvinh/bookings/internal/repository/dbrepo"
	"log"
	"net/http"
	"strconv"
	"strings"
...
// AvailabilityJSON handles request for availability and send JSON page
...
// AdminPostReservationsCalendar handlers post of reservation calendar
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				...
				//	 the res are just placeholders for days without block
				if val > 0 {
	...
	for name := range r.PostForm {
		if strings.HasPrefix(name, "add_block") {
			exloded := strings.Split(name, "_")
			roomID, _ := strconv.Atoi(exloded[2])
			t, _ := time.Parse("2006-01-2", exloded[3])
			//	insert a new block
